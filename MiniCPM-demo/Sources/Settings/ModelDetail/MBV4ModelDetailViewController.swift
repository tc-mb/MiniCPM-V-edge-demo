//
//  MBV4ModelDetailViewController.swift
//  MiniCPM-demo
//
...
/// v4模型详情页面 VC
@objc public class MBV4ModelDetailViewController: UIViewController, UIGestureRecognizerDelegate {
    
    /// 模型名称
    var modelName: String = "MiniCPM-V 4.0 4B"
    
    /// V4 下载管理器
    private let downloadManager = MBV4ModelDownloadManager.shared
    ...
    private func deleteAllDownloadedFiles() {
        ...
        // 删除V4相关的所有文件
        let filesToDelete = [
            MiniCPMModelConst.modelv4_Q4_K_M_FileName,
            MiniCPMModelConst.mmprojv4_FileName,
            MiniCPMModelConst.mlmodelcv4_ZipFileName
        ]
    ...
    private func updateCellProgress(modelName: String, progress: CGFloat) {
        ...
        debugLog("-->> V4 DetailVC: 收到进度回调 - 模型: \(modelName), 进度: \(progressPercentage)")
...
extension MBV4ModelDetailViewController: UITableViewDataSource {
...
extension MBV4ModelDetailViewController: UITableViewDelegate {
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if let title = model.title {
            switch title {
            case MiniCPMModelConst.modelv4_Q4_K_M_DisplayedName:
                handleModelDownload(modelName: title, downloadAction: { [weak self] in
                    self?.downloadManager.downloadModelv4_Q4_K_M()
                })
...
// MARK: - 数据配置
extension MBV4ModelDetailViewController {
    ...
    public func loadTableViewData() {
        ...
        languageModel.title = MiniCPMModelConst.modelv4_Q4_K_M_DisplayedName
        languageModel.icon = UIImage(systemName: "cpu")
        languageModel.statusString = getInitialStatus(for: downloadManager.getModelv4_Q4_K_M_Status())
        languageModel.shouldShowStatusText = true
        dataArray.append(languageModel)
        
        // 多模态模型
        let multimodalModel = MBSettingsModel()
        multimodalModel.title = MiniCPMModelConst.modelMMProjv4_DisplayedName
    ...
    private func checkAllModelsDownloaded() -> Bool {
        let mainModelStatus = downloadManager.getModelv4_Q4_K_M_Status()
        let vitModelStatus = downloadManager.getMMProjv4_Status()
        let aneModelStatus = downloadManager.getMLModelcv4_Status()
    ...
    private func setAsCurrentModel() {
        ...
        // 保存到UserDefaults
        UserDefaults.standard.setValue("V4MultiModel", forKey: "current_selected_model")
